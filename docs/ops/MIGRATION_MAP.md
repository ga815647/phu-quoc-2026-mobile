...
- **未處置、不計入無缺漏**：(1) 網站獨有文字（暫排下拉選項、交通分段說明、空狀態文案）留程式碼，未搬遷——屬生成邏輯，無需搬；(2) 裝置 localStorage（暫排、吃過）未搬遷亦未放棄，匯出步驟見 `LOCALSTATE_EXPORT.md`，需使用者裝置執行；(3) 私人 Notion 頁保留原處（日常來源，不搬進 Neon）；(4) 舊研究樹封存。
