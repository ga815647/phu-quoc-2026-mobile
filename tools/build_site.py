...
<details class="src-info" id="srcInfo" style="margin-top:12px"><summary>資料資訊</summary><div id="srcDetail">失敗時顯示備援資料{fb_day}</div></details>
...
function setDsrc(kind,when){{var fb=document.getElementById('srcFallback');var ln=document.getElementById('srcLine');var dt=document.getElementById('srcDetail');if(kind!=='api'){{if(fb)fb.hidden=false}}if(ln)ln.textContent=(kind==='api'?('卡片資料 · {L_D_API}'+when):('{L_D_FB}'+when));if(dt&&kind!=='api')dt.textContent='失敗時顯示備援資料 · 快照 '+when}}
...
<details class="panel pad depart-check" style="margin-top:16px"><summary>出發前確認</summary><div class="prep-grid"><div class="prep"><b>OnBird</b><span>接送時間、當天海況、是否照常出發；現場以教練安全評估為準。</span></div><div class="prep"><b>纜車</b><span>10 月第一波時段、回程與末班、是否維修、天氣。</span></div><div class="prep"><b>VinWonders</b><span>營業與餐飲時段、票價、兒童設施限制。</span></div><div class="prep"><b>海星</b><span>前 48 小時確認路況、海星、水色、船與回程；七項條件全通過才去。</span></div><div class="prep"><b>Safari</b><span>營業時間、直達叫車與北島回程；往 Gành Dầu 先談好回程並於 17:00 前離開。</span></div><div class="prep"><b>用餐</b><span>只查明天可能吃的店：是否營業、想點的品項與價格。</span></div></div></details>
</section>
