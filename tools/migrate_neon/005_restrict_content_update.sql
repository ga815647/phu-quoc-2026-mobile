...
--
-- 正式套用前需確認 Chat connector 實際身分。2026-09-22 ChatGPT 自查回報：
-- effective_role=neondb_owner，login_role=neondb_owner，
-- can_execute_content_update=true；與 OpenCode 側 current_user/session_user
-- 查證一致。故正式套用不需新增 GRANT（owner 執行權來自擁有者身分，
-- 不依賴 PUBLIC 授權）。正式 ACL 變更仍依工具要求走確認流程，不逕行套用。
--
-- 若未來維護者為其他具寫入權角色，套用本檔後再對該角色補精確
-- signature 的 GRANT（不擴大表權限）：
